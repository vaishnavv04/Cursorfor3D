/*
 * generate_from_polyhaven.js
 *
 * A standalone script to demonstrate the full orchestration flow for PolyHaven:
 * 1. Connect to Blender TCP server.
 * 2. Check PolyHaven status.
 * 3. Search for a model by query to get its Asset ID.
 * 4. Download and import the first available model by its Asset ID.
 ...
 * 3. Run: `node generate_from_polyhaven.js`
 */
...
// --- Configuration ---
const USER_PROMPT = "Lion head"; // A good generic prompt for PolyHaven
const PORT = parseInt(process.env.BLENDER_TCP_PORT || "9876", 10);
const HOST = process.env.BLENDER_TCP_HOST || "127.0.0.1";
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const GEMINI_MODEL = "gemini-pro"; // Using gemini-pro as requested
...
function sendCommand(commandType, params = {}) {
    return new Promise((resolve, reject) => {
        ...
        if (commandType.startsWith('download_')) {
...
async function runGeneration() {
    try {
        console.log(`\n--- 🚀 Starting PolyHaven Generation ---`);
        console.log(`Prompt: "${USER_PROMPT}"`);
        
        // 1. Check PolyHaven Status
        console.log('\n[Step 1/5] Checking PolyHaven status...');
        const status = await sendCommand('get_polyhaven_status');
        if (!status.enabled) {
            throw new Error('PolyHaven integration is not enabled.');
        }
        console.log(`   -> PolyHaven is ENABLED. (${status.message})`);

        // 2. Search for Model to get Asset ID
        console.log('\n[Step 2/5] Searching PolyHaven for models...');
        
        // ******************************************************
        // THE FIX IS HERE:
        // Filter out useless words and join with a comma
        // ******************************************************
        const stopWords = ['a', 'an', 'the', 'of', 'for', 'with'];
        const keywords = USER_PROMPT.split(' ')
                                    .map(w => w.toLowerCase())
                                    .filter(w => !stopWords.includes(w) && w.length > 1);
        const categoryString = keywords.join(','); // Creates "wooden,chair"

        console.log(`   -> Searching with categories: "${categoryString}"`);
        
        const searchResult = await sendCommand('search_polyhaven_assets', { 
            asset_type: "models",
            categories: categoryString // Send the correct string
        });


        if (!searchResult.assets || Object.keys(searchResult.assets).length === 0) {
            throw new Error(`No models found on PolyHaven for query: "${USER_PROMPT}" (categories: ${categoryString})`);
        }
        
        // Get the first key (asset_id) from the assets object
        const assetId = Object.keys(searchResult.assets)[0];
        const modelName = searchResult.assets[assetId].name;
        console.log(`   -> Found model: "${modelName}" (ID: ${assetId})`);
        
        // 3. Download and Import Model
        console.log('\n[Step 3/5] Downloading and importing model from PolyHaven...');
        const importResult = await sendCommand('download_polyhaven_asset', {
            asset_id: assetId,
            asset_type: "models",
            resolution: "1k", // 1k is usually fastest
            file_format: "gltf" // glTF is preferred
        });

        if (!importResult.success || !importResult.imported_objects || importResult.imported_objects.length === 0) {
            throw new Error(`Failed to import PolyHaven model: ${importResult.error || 'No objects were imported'}`);
        }
...
4. Scale the object uniformly so its largest dimension is 1.5 meters.
5. Move the object so its center is at (0, 0, 0.75), placing it 0.75m above the grid floor.`;
